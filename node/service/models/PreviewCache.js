...
class PreviewCache {
  constructor(projectId, workspaceId, requestor) {
    this.projectId = projectId;
    this.workspaceId = workspaceId;
    this.keyBase = `workspace:${this.workspaceId}:preview_cache:`;
    this.client = redis();
    ...
    this.outputPath = setupVal(this, 'output_path', {
      keyPrefix: this.keyBase,
    });
  ...
  cache(workspaceName, entry, outputPath, data) {
    // TODO: Pipeline this redis work
    this.client.set(`${this.keyBase}workspace_name`, workspaceName, 'EX', expireation);
    this.client.set(`${this.keyBase}output_path`, outputPath, 'EX', expireation);
  ...
  renew() {
    ...
    this.client.expire(`${this.keyBase}output_path`, expireation);
